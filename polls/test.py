...
class MySeleniumTests(StaticLiveServerTestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        opts = Options()
        opts.headless = True  # Activamos el modo headless para no usar entorno gráfico
    ...
    def test_crear_preguntas_y_choices(self):
        ...
        choice_text_field = self.selenium.find_element(By.NAME, 'inline_admin_formset-0-choice_text')
        choice_text_field.send_keys('Choice 1 per Pregunta 1')
        save_button = self.selenium.find_element(By.NAME, "_addanother")
        save_button.click()

        # Afegim la segona opció per la Pregunta 1
        choice_text_field = self.selenium.find_element(By.NAME, 'inline_admin_formset-1-choice_text')
